...
# AR(1) SAMPLED Z SENSITIVITY ANALYSIS
...
OUTDIR = Path(
    r"F:\gacheri's documents\research NCDs\BIBTEX\New folder\Simulated Datasets\Main Sensitivity\AR1_sampledZ_results"
)
...
Z_PRIOR_SPECS = [
    {
        "name": "Beta_1_1_sampled",
        "alpha": 1.0,
        "beta": 1.0
    },
    {
        "name": "Beta_2_2_sampled",
        "alpha": 2.0,
        "beta": 2.0
    }
]
...
def scenario_tag(z_prior_name, mu_prior_sd, sigma_prior_name):
    return f"{clean_label(z_prior_name)}_mu{mu_prior_sd}_{sigma_prior_name}"
...
def ar1_latent_covariance(sigma_omega, rho):

    rho2 = rho**2
    rho3 = rho**3
    rho4 = rho**4

    Sigma = sigma_omega**2 * pt.stack([
...
def save_traceplot(trace, z_prior_name, mu_prior_sd, sigma_prior_name, scenario_number):
    tag = scenario_tag(z_prior_name, mu_prior_sd, sigma_prior_name)
...
def create_parameter_comparison_tables(final_df):
    ...
    excel_path = TABLE_DIR / "AR1_constrainedZ_parameter_comparison_tables.xlsx"
    ...
    try:
        with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
            ...
            sigma_summary_df.to_excel(writer, sheet_name="Sigma prior sensitivity", index=False)
...
def plot_scenario_ranking(ranking_df):
    ...
    plt.ylabel("Scenario")
    plt.title("Top 15 AR(1) constrained-Z scenarios by average RMSE")
...
def plot_learned_z_by_prior(z_summary_df):
    ...
    plt.xlabel("Z prior")
    plt.title("Learned constrained Z by prior")
    plt.grid(axis="y", alpha=0.3)
...
def main():
    ...
    print("\n================================================")
    print("AR(1) SAMPLED-Z SENSITIVITY ANALYSIS")
    print("Marginal AR(1) likelihood; no latent-state sampling")
    print("Z sampled from Beta and sampled deterministically")
    print("================================================")
    ...
    Y_groups = group_rows_by_missing_pattern(Y)
    C_groups = group_rows_by_missing_pattern(C)
            ...
            for sigma_prior_spec in SIGMA_PRIOR_SPECS:
                ...
                try:
                    ...
                    scenario_path = OUTDIR / (
                        f"scenario_{scenario_number}_{z_prior_name}_mu{mu_prior_sd}_{sigma_name}.csv"
                    )
                    ...
                    progress_path = OUTDIR / "AR1_sampledZ_progress.csv"
                ...
                diagnostics_path = OUTDIR / "AR1_sampledZ_diagnostics.csv"
    ...
    if all_results:
        ...
        final_path = OUTDIR / "AR1_sampledZ_final_results.csv"
        ...
        ranking_path = OUTDIR / "AR1_sampledZ_scenario_ranking.csv"
        ...
        z_summary_path = OUTDIR / "AR1_sampledZ_Z_summary.csv"
        ...
        print(OUTDIR / "AR1_sampledZ_progress.csv")
        print(OUTDIR / "AR1_sampledZ_diagnostics.csv")
        print(TRACE_DIR)
        print(TRACEPLOT_DIR)
