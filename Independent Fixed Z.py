...
# INDEPENDENT STRUCTURE FIXED-Z SENSITIVITY ANALYSIS
...
OUTDIR = Path(
    r"F:\gacheri's documents\research NCDs\BIBTEX\New folder\Simulated Datasets\Main Sensitivity\Independent_fixedZ_results"
)
...
def scenario_tag(z_value, mu_prior_sd, sigma_name):
    return f"Z{clean_label(z_value)}_mu{mu_prior_sd}_{sigma_name}"
...
def fit_one_scenario(
    ...
    with pm.Model() as model:
        ...
        pm.Potential(
            ...
            (1.0 - z_value) * pm.math.sum(c_logp)
        )
...
def save_traceplot(trace, z_value, mu_prior_sd, sigma_name, scenario_number):
    tag = scenario_tag(z_value, mu_prior_sd, sigma_name)
...
def create_parameter_comparison_tables(parameter_df):
    ranking_df = (
        parameter_df
        .groupby(
    ...
    excel_path = TABLE_DIR / "Independent_fixedZ_parameter_comparison_tables.xlsx"
...
def plot_scenario_ranking(ranking_df):
    ...
    plt.ylabel("Scenario")
    plt.title("Top 15 independent fixed-Z scenarios by average RMSE")
...
def generate_sensitivity_plots(parameter_df, tables):
...
def main():
    ...
    print("\n================================================")
    print("INDEPENDENT STRUCTURE FIXED-Z SENSITIVITY ANALYSIS")
    print("Marginal independent likelihood; no latent-state sampling")
    print("Z fixed at 0.90, 0.70, and 0.50")
    print("================================================")
            ...
            for sigma_prior_spec in SIGMA_PRIOR_SPECS:
                ...
                try:
                    ...
                    scenario_path = OUTDIR / (
                        f"scenario_{scenario_number}_Z{z_tag}_mu{mu_prior_sd}_{sigma_name}.csv"
                    )
                    ...
                    progress_path = OUTDIR / "Independent_fixedZ_progress.csv"
                ...
                diagnostics_path = OUTDIR / "Independent_fixedZ_diagnostics.csv"
    ...
    if all_results:
        ...
        final_path = OUTDIR / "Independent_fixedZ_final_results.csv"
        ...
        tables = create_parameter_comparison_tables(parameter_df)
        generate_sensitivity_plots(parameter_df, tables)

        ranking_df = tables["ranking"]

        ranking_path = OUTDIR / "Independent_fixedZ_scenario_ranking.csv"
        ...
        print(final_path)
        print(ranking_path)
        print(OUTDIR / "Independent_fixedZ_progress.csv")
        print(OUTDIR / "Independent_fixedZ_diagnostics.csv")
        print(TRACE_DIR)
        print(TRACEPLOT_DIR)
